...
public class zzkr extends zzkw {
    ...
    class C05861 implements OnClickListener {
        final /* synthetic */ zzkr zzMl;

        C05861(zzkr com_google_android_gms_internal_zzkr) {
    ...
    private void zzj(Map<String, String> map) {
        if (!TextUtils.isEmpty((CharSequence) map.get(SettingsJsonConstants.ICON_WIDTH_KEY))) {
            this.zzrC = zzw.zzcM().zzaY((String) map.get(SettingsJsonConstants.ICON_WIDTH_KEY));
        }
        if (!TextUtils.isEmpty((CharSequence) map.get(SettingsJsonConstants.ICON_HEIGHT_KEY))) {
            this.zzrD = zzw.zzcM().zzaY((String) map.get(SettingsJsonConstants.ICON_HEIGHT_KEY));
        }
