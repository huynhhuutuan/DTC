...
public class zzy extends Thread implements OnFrameAvailableListener, zza {
    ...
    private EGLConfig zzis() {
        ...
        return !this.zzOP.eglChooseConfig(this.zzOQ, new int[]{12352, 4, 12324, 8, 12323, 8, 12322, 8, 12325, 16, 12344}, eGLConfigArr, 1, iArr) ? null : iArr[0] > 0 ? eGLConfigArr[0] : null;
    }

    private String zzit() {
        zzfz com_google_android_gms_internal_zzfz = zzgd.zzDo;
        return !((String) com_google_android_gms_internal_zzfz.get()).equals(com_google_android_gms_internal_zzfz.zzfr()) ? (String) com_google_android_gms_internal_zzfz.get() : "attribute highp vec3 aPosition;varying vec3 pos;void main() {  gl_Position = vec4(aPosition, 1.0);  pos = aPosition;}";
    }

    private String zziu() {
        zzfz com_google_android_gms_internal_zzfz = zzgd.zzDp;
        return !((String) com_google_android_gms_internal_zzfz.get()).equals(com_google_android_gms_internal_zzfz.zzfr()) ? (String) com_google_android_gms_internal_zzfz.get() : "#extension GL_OES_EGL_image_external : require\n#define INV_PI 0.3183\nprecision highp float;varying vec3 pos;uniform samplerExternalOES uSplr;uniform mat3 uVMat;uniform float uFOVx;uniform float uFOVy;void main() {  vec3 ray = vec3(pos.x * tan(uFOVx), pos.y * tan(uFOVy), -1);  ray = (uVMat * ray).xyz;  ray = normalize(ray);  vec2 texCrd = vec2(    0.5 + atan(ray.x, - ray.z) * INV_PI * 0.5, acos(ray.y) * INV_PI);  gl_FragColor = vec4(texture2D(uSplr, texCrd).xyz, 1.0);}";
    }
    ...
    public void run() {
        Object obj = 1;
        if (this.zzOI == null) {
            zzqf.m9e("SphericalVideoProcessor started with no output texture.");
            this.zzON.countDown();
            return;
        }
        boolean zzir = zzir();
        ...
        if (!zzir || r0 == null) {
            ...
            zzqf.m9e(valueOf);
            zzw.zzcQ().zza(new Throwable(valueOf), "SphericalVideoProcessor.run.1");
            zziv();
            this.zzON.countDown();
            return;
        }
        ...
        try {
            ...
            zzqf.zzbh("SphericalVideoProcessor halted unexpectedly.");
        } catch (Throwable th) {
            zzqf.zzb("SphericalVideoProcessor died.", th);
    ...
    boolean zzir() {
        ...
        return (this.zzOS == null || this.zzOS == EGL10.EGL_NO_SURFACE) ? false : this.zzOP.eglMakeCurrent(this.zzOQ, this.zzOS, this.zzOS, this.zzOR);
    }

    boolean zziv() {
        boolean z = false;
        if (!(this.zzOS == null || this.zzOS == EGL10.EGL_NO_SURFACE)) {
            z = (this.zzOP.eglMakeCurrent(this.zzOQ, EGL10.EGL_NO_SURFACE, EGL10.EGL_NO_SURFACE, EGL10.EGL_NO_CONTEXT) | 0) | this.zzOP.eglDestroySurface(this.zzOQ, this.zzOS);
            this.zzOS = null;
        }
        if (this.zzOR != null) {
        ...
        z |= this.zzOP.eglTerminate(this.zzOQ);
        this.zzOQ = null;
        return z;
    }
