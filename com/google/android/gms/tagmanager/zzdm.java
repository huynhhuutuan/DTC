...
public class zzdm extends zzdj {
    ...
    private static final String zzbIP = zzai.ACCOUNT.toString();
    private static final String zzbIQ = zzai.ANALYTICS_PASS_THROUGH.toString();
    private static final String zzbIR = zzai.ENABLE_ECOMMERCE.toString();
    private static final String zzbIS = zzai.ECOMMERCE_USE_DATA_LAYER.toString();
    private static final String zzbIT = zzai.ECOMMERCE_MACRO_DATA.toString();
    private static final String zzbIU = zzai.ANALYTICS_FIELDS.toString();
    private static final String zzbIV = zzai.TRACK_TRANSACTION.toString();
    private static final String zzbIW = zzai.TRANSACTION_DATALAYER_MAP.toString();
    private static final String zzbIX = zzai.TRANSACTION_ITEM_DATALAYER_MAP.toString();
    private static final List<String> zzbIY = Arrays.asList(new String[]{ProductAction.ACTION_DETAIL, ProductAction.ACTION_CHECKOUT, ProductAction.ACTION_CHECKOUT_OPTION, "click", ProductAction.ACTION_ADD, ProductAction.ACTION_REMOVE, ProductAction.ACTION_PURCHASE, ProductAction.ACTION_REFUND});
    ...
    zzdm(Context context, DataLayer dataLayer, zzdi com_google_android_gms_tagmanager_zzdi) {
        ...
        this.zzbJe = com_google_android_gms_tagmanager_zzdi;
    ...
    private void zza(Tracker tracker, Map<String, zza> map) {
        ...
        if (zzhB == null) {
            zzbo.m11e("Cannot find transactionId in data layer.");
            return;
        }
        List<Map> linkedList = new LinkedList();
        try {
            Map zzk = zzk((zza) map.get(zzbIU));
            ...
            if (zzhC != null) {
                for (Map map2 : zzhC) {
                    if (map2.get("name") == null) {
                        zzbo.m11e("Unable to send transaction item hit due to missing 'name' field.");
                        return;
                    }
                    Map zzk2 = zzk((zza) map.get(zzbIU));
                    zzk2.put("&t", "item");
                    zzk2.put("&ti", zzhB);
    ...
    private Product zzak(Map<String, Object> map) {
        ...
        for (String valueOf2 : map.keySet()) {
            Matcher matcher = zzbIZ.matcher(valueOf2);
            if (matcher.matches()) {
                try {
                ...
                matcher = zzbJa.matcher(valueOf2);
                if (matcher.matches()) {
                    try {
    ...
    private Map<String, String> zzal(Map<String, zza> map) {
        zza com_google_android_gms_internal_zzak_zza = (zza) map.get(zzbIW);
    ...
    private Map<String, String> zzam(Map<String, zza> map) {
        zza com_google_android_gms_internal_zzak_zza = (zza) map.get(zzbIX);
        if (com_google_android_gms_internal_zzak_zza != null) {
            return zzc(com_google_android_gms_internal_zzak_zza);
        }
        if (zzbJc == null) {
    ...
    private void zzb(Tracker tracker, Map<String, zza> map) {
        ...
        if (zzi(map, zzbIS)) {
            obj = this.zzbEZ.get("ecommerce");
            map2 = obj instanceof Map ? (Map) obj : null;
        } else {
            obj = zzdl.zzj((zza) map.get(zzbIT));
            map2 = obj instanceof Map ? (Map) obj : null;
        }
        if (map2 != null) {
            Map map3;
            ...
            if (obj instanceof List) {
                for (Map map4 : (List) obj) {
                    try {
                        ...
                        zzbo.m11e(str2.length() != 0 ? str.concat(str2) : new String(str));
                    }
                }
            }
                        ...
                        for (String str22 : zzbIY) {
                            if (map2.containsKey(str22)) {
                                ...
                                try {
                                    ...
                                    zzbo.m11e(str22.length() != 0 ? str3.concat(str22) : new String(str3));
                                }
                            }
                        }
                    }
    ...
    public /* bridge */ /* synthetic */ boolean zzQa() {
        return super.zzQa();
    }
    ...
    public void zzab(Map<String, zza> map) {
        Tracker zzht = this.zzbJe.zzht("_GTM_DEFAULT_TRACKER_");
        zzht.enableAdvertisingIdCollection(zzi(map, "collect_adid"));
        if (zzi(map, zzbIR)) {
            zzb(zzht, map);
        } else if (zzi(map, zzbIQ)) {
            zzht.send(zzk((zza) map.get(zzbIU)));
        } else if (zzi(map, zzbIV)) {
            zza(zzht, map);
        } else {
